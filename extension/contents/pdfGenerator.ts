...
export const generatePDF = async (
  ...
  const renderFormattedBlock = (
     ...
     options?: { borderLeftColor?: string }
      ...
      let currentPageYStart = startY
      
      // Helper to handle page breaks
      const checkAndAddPage = (currentY: number, lh: number): number => {
          if (currentY + lh > config.pageHeight - config.margin - 10) { 
              // Draw border for the current page chunk before breaking
              if (options?.borderLeftColor && targetDoc === doc) {
                  targetDoc.setDrawColor(options.borderLeftColor)
                  targetDoc.setLineWidth(1)
                  // Draw from start of this page's block to the current Y (plus a bit of padding)
                  const lineBottom = currentY + lh - 2 // Adjust end
                  targetDoc.line(startX - 5, currentPageYStart - 2, startX - 5, lineBottom)
              }
              ...
              const newY = config.margin
              currentPageYStart = newY // Reset start for new page
              return newY
          }
          return currentY
      }
      ...
      const drawFinalBorder = (endY: number) => {
          if (options?.borderLeftColor && targetDoc === doc) {
              targetDoc.setDrawColor(options.borderLeftColor)
              targetDoc.setLineWidth(1)
              targetDoc.line(startX - 5, currentPageYStart - 2, startX - 5, endY)
          }
      }
  ...
  // Add Notes Page to TOC (It will be at the end, after content pages)
  tocEntries.push({ 
      text: 'NOTES', 
